...
from keras_frcnn.simple_parser_logood import get_data
...
from keras_frcnn import nn_arch_vgg16
...
def Train_frcnn(train_path='./data1/',  # path to the text file containing the data
    ...
    classifier = nn.classifier(shared_layers, roi_input, C.num_rois, len(classes_count),
    ...
    data_gen_train = data_generators.get_anchor_gt(train_imgs, classes_count, C, model_base, K.image_dim_ordering(),
    ...
    data_gen_val = data_generators.get_anchor_gt(val_imgs, classes_count, C, model_base, K.image_dim_ordering(),
                                                 preprocessing_function, mode='val')
        ...
        while True:
            try:
                ...
                R = roi_helpers.rpn_to_roi(P_rpn[0], P_rpn[1], C, K.image_dim_ordering(), use_regr=True,
                                           overlap_thresh=C.rpn_nms_threshold, flag="train")
                ...
                if iter_num == epoch_length:
                    ...
                    loss_dict_train = {"rpn_cls": loss_rpn_cls, "rpn_reg": loss_rpn_regr, "final_cls": loss_class_cls,
                    ...
                    if curr_loss < best_loss:
                        if C.verbose:
                            print('Total loss decreased from {} to {} in training, saving weights'.format(best_loss,
                                                                                                          curr_loss))
                            save_log_data = '\nTotal loss decreased from {} to {} in epoch {}/{} in training, saving weights'.format(
                                best_loss, curr_loss, epoch_num + 1, num_epochs)
                    ...
                    try:
                        ...
                        if C.num_rois > 1:
                            ...
                            if np.random.randint(0, 2):
                                ...
                                sel_samples = random.choice(pos_samples)
                        if train_final_classifier:
                            val_loss_class = model_classifier.test_on_batch([X, X2[:, sel_samples, :]],
                                                                            [Y1[:, sel_samples, :],
                                                                             Y2[:, sel_samples, :]])
                        ...
                        if iter_num == validation_epoch_length:
                            ...
                            loss_dict_valid = {"rpn_cls": val_loss_rpn_cls, "rpn_reg": val_loss_rpn_regr,
                                               "final_cls": val_loss_class_cls, "final_reg": val_loss_class_regr}
                            ...
                            write_log(tbCallBack, val_names,
                                      [val_loss_rpn_cls, val_loss_rpn_regr, val_loss_class_cls, val_loss_class_regr,
                                       val_curr_loss, val_class_acc], epoch_num)
                            ...
                            if C.verbose:
                                ...
                                val_curr_loss, val_best_loss, val_best_loss_epoch))
                                print('Elapsed time: {}'.format(time.time() - start_time))

                            if val_curr_loss < val_best_loss:
                                if C.verbose:
                                    print('Total loss decreased from {} to {}, saving weights'.format(val_best_loss,
                                                                                                      val_curr_loss))
                                    save_log_data = '\nTotal loss decreased from {} to {} in epoch {}/{} in validation, saving weights'.format(
                                        val_best_loss, val_curr_loss, epoch_num + 1, num_epochs)
                                    with open("./saving_log.txt", "a") as f:
                                        f.write(save_log_data)
